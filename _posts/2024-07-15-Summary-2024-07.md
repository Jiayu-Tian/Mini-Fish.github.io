...
**Decision Variable**: the Stocking Quantity $x$, Sales Price $p=p(d)$
 ...
 $(x, d)^* =\underset{x, d}{\operatorname{argmax\quad }} \pi(x, d)$
...
$V_t(I_t)=cI_t+\underset{x_t, d_t}{\operatorname{max\quad }}[\pi(x_t, d_t)+\alpha V_{t-1}(I_{t-1})]=cI_t+\underset{x_t, d_t}{\operatorname{max\quad }}J_t(x_t, d_t)$
...
$\hat{V}^l_t(I_t)=\underset{x_t, d_t}{\operatorname{max\quad }}\hat{J}^l_t(\mathbf{I_t},q_t, d_t)$
